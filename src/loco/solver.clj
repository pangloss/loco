...
(defn- set-search-monitor-settings! [solver named-params]
  (doseq [[method args] named-params]
    (case method
      ...
      (:max :limit-solution :limitSolution)                  ;; void <- limitSolution(long limit)
      (.limitSolution solver args)
      (:timeout :limit-time :limitTime)                      ;; void <- limitTime(long limit) | (String duration)
      (.limitTime solver args)
      (:no-good-from-restart :setNoGoodRecordingFromRestarts) ;; void <- setNoGoodRecordingFromRestarts()
      (.setNoGoodRecordingFromRestarts solver)
      (:restart-on-solution :setRestartOnSolutions)
      (.setRestartOnSolutions solver)
      nil)))
