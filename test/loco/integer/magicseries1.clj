...
(deftest magicseries-tests
  ...
  (is (empty? (solver/solutions (ms-model 1))) "Length 1 does not have solution")
  (is (empty? (solver/solutions (ms-model 2))) "Length 2 does not have solution")
  (is (empty? (solver/solutions (ms-model 3))) "Length 3 does not have solution")

  ;; No solution for length = 6 (try it out!)
  (is (empty? (solver/solutions (ms-model 6))) "Length 6 does not have solution")

  ;; Testing between 4 (inclusive) and 20 (inclusive)
  (doseq [l (range 4 21)]
    (doseq [sol (solver/solutions (ms-model l))]
